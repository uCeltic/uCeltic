 ...
 */
function searchCompleted(search: SearchSignalState): boolean {
 ...
 */
function resultNavigated(search: SearchSignalState): boolean {
 ...
 *
 * The one seam between the stores and the tour, so the derivation itself stays
 * a pure function over four values and can be tested without a store or an
 * overlay in sight. Nothing is recorded for the tour's benefit — every signal is
 * read off state the workspace already keeps for its own reasons.
 */
export function tourSignalsFrom(
  documents: Pick<DocumentState, "openDocuments">,
  search: SearchSignalState,
  workspace: Pick<WorkspaceState, "fontSize">,
): TourSignals {
  return {
    openDocumentCount: documents.openDocuments.length,
    searchCompleted: searchCompleted(search),
    resultNavigated: resultNavigated(search),
    fontSizeChanged: workspace.fontSize !== DEFAULT_FONT_SIZE,
  };
}
 ...
 *
 * Each store is selected down to a primitive, and the object is rebuilt only
